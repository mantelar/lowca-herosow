...
(function () {
    ...
    const showHeroDropdown = (anchorElement) => {
        ...
        const playerLevel = lastPlayerLevel;
        const allHeroes = getHeroList().sort((a, b) => b.level - a.level);
    ...
    const updateTable = () => {
        console.log('[DEBUG] updateTable wywołane');
        ...
        console.log('[DEBUG] Wiersze do wyświetlenia:', rows);
    ...
    const createUI = () => {
        ...
        if (!toggleBtn) {
            ...
            btn.style.cssText = `
                position: fixed;
                bottom: ${GM_getValue('toggle_top', '10px')};
                left: ${GM_getValue('toggle_left', '10px')};
                width: 25px;
                height: 25px;
                background: black;
                color: white;
